...
public class StatusViewViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenAddordered_then_CREATE_2 (@Payload Addordered addordered) {
        try {
            if (addordered.isMe()) {
                // view 객체 생성
                StatusView statusView = new StatusView();
                // view 객체에 이벤트의 Value 를 set 함
                statusView.setOrderId(addordered.getId());
                statusView.setOrderStatus(addordered.getAddorderstatus());
                // view 레파지 토리에 save
                statusViewRepository.save(statusView);
            }
        }catch (Exception e){
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenWritten_then_UPDATE_5(@Payload Written written) {
        try {
            if (written.isMe()) {
                // view 객체 조회
                List<StatusView> statusViewList = statusViewRepository.findByOrderId(written.getOrderedId());
                for(StatusView statusView : statusViewList){
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    statusView.setReviewtext(written.getReviewtext());
                    // view 레파지 토리에 save
                    statusViewRepository.save(statusView);
                }
            }
